            ...
            <div className="w-full flex">
                <AnimationWrapper className="rounded-lg w-full grid-flow-row grid grid-cols-1 sm:grid-cols-3 py-9 divide-y-2 sm:divide-y-0 sm:divide-x-2 divide-green-main bg-ehite-500 z-10">
                    {aboutDataInfo.map((infoItem, index)=>(
                        ...
                        ${
                            index === 0 ? "sm:justify-start" : index === 1 ? "sm:justify-center" : "sm:justify-end"
            ...
            <AnimationWrapper className={"pt-6"}>
                <div className="flex flex-col justify-center items-center row-start-2 sm:row-start-1">
                    ...
                    {headingText.split(" ").map((item, index)=>(
                        <span className={`${index === 6 ? "text-green-main" : "text-black"}`}> {item}{" "}</span>
                    ))}
                    </h1>
            ...
            <div className="grid grid-flow-row sm:grid-flow-col grid-cols-1 sm:grid-cols-2 gap-8">
                ...
                <AnimationWrapper className={"flex items-center w-full p-4"}>
                    ...
                    {data?.skills.split(",").map((skills)=>(
                        <motion.div className="w-full justify-center items-center" variants={skillItemVariant}>
                            <button className="whitespace-nowrap text-ellipsis overflow-hidden py-3 w-[150px] px-6 border-[2px] border-green-main bg-white text-black font-bold rounded-lg text-xl tracking-widest hover:shadow-green-main transition-all outline-none">
                                {skills}
                            </button>
                        </motion.div>
                    ))}
